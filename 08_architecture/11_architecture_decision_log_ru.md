...
| ADR-034 | Compat gateway is the primary path for the two owned WP sites | accepted | For Lampyris and Vino&Co, the Bil24-compatible gateway (`hbil24`/`bil24compat`) is the primary integration path; guardrail #6 ("native plugin is the target path") and ADR-011/012/013 (hosted checkout, WooCommerce bypass, `arena_event` CPT) do not apply to these two sites. | Owner decision 2026-09-04 #1 (`docs/migration/wp_sites_to_arena_wave1_2026-09-04.md` §7): the sites already run Bil24-shaped PHP (`bil24-acf-sync`, `bil24-cart-bridge`, …) that cannot be rewritten in this wave; wire-compatible drop-in is the only migration path that fits the timeline. See `08_architecture/18_bil24_compat_wave1_specification_ru.md` §17. |
| ADR-035 | `orders` aggregate / `ordering` package is the sole order-mutation path | accepted | The `ordering` package (`ordering.CreateOrderFromCheckout` and friends) is the only place that creates, pays or cancels an order, for every surface (buyer checkout, admin, compat gateway). Partially reverses ADR-033 by carving one aggregate-shaped exception out of the handler-centric layout for order lifecycle specifically. | The compat gateway's `CREATE_ORDER_EXT`/`PAY_ORDER`/`CANCEL_ORDER` must produce byte-identical invariants (`orders.status`, `order_items`, `payment_intents`, ticket issuance, outbox events) to the native buyer checkout; duplicating that logic per-surface is how W1's "one open order per session" and reacquire-on-expired-hold rules would drift. Spec §17, §7.7–§7.9. |
| ADR-036 | Customer is a platform entity; consent is per-organization | accepted | `customers` is a platform-level entity (not org-scoped); marketing/communication consent lives per `(customer, org)` via `customer_org_links`/`consent` records. Strong identity keys (verified email/phone) are never auto-merged across import sources or organizations. | Owner decision #11: multiple organizers may see the same buyer, but an organizer must not inherit another organizer's marketing consent, and an automated merge on a weak match (name-only, fuzzy) risks conflating two different people. `customer.import` (feature #519) enforces `customers.Resolve` without auto-merge and `consent.source='import:<label>'` staying unverified. Spec §17, §12. |
| ADR-037 | Wire-integer-ID rule for the compat gateway (spec §4) | accepted | Catalog objects (`actionId`, `venueId`, `categoryPriceId`, …) are exposed via a `compatids` int64 map table; ticket/seat/order/customer ids reuse existing bigint columns (`system_ticket_id`, `system_seat_id`, …); ids that originated in legacy Bil24 keep their original numeric value; new arena-native ids are minted from a sequence starting at `1e9` so they never collide with a real historical Bil24 id. | Legacy PHP clients parse ids as JS/PHP integers and some templates do `(int)$id` (see the `fid` UUID-truncation bug in spec §0); every id on the wire must be a genuine `int64`, and preserving legacy numeric ids lets already-issued paper/PDF tickets and cached site data keep resolving after cutover. Spec §17, §4. |
| ADR-038 | Org service keys — bcrypt, permission-shaped scopes, shown once | accepted | First half of ADR-029 (API security model) made concrete for W1: organization service keys are bcrypt-hashed at rest, their scopes are drawn from the same permission catalog as user roles (no separate scope vocabulary), every key is hard-bound to one org, and the plaintext key is returned exactly once at creation (never re-displayed, never logged). | Needed so the compat gateway's channel credentials (`fid`/`token`) and the admin-web API-key management page (feature #473/#474) share one security model instead of inventing a second one; matches the "scoped service accounts first" direction ADR-029 already set. Spec §17. |
