// Hand-maintained Querier interface; follows sqlc output conventions.
...
type Querier interface {
	...
	ChangeMembershipRole(ctx context.Context, id, orgID uuid.UUID, role string) (MembershipRow, error)
	DeactivateMembership(ctx context.Context, id, orgID uuid.UUID) (MembershipRow, error)
	...
	InsertCountry(ctx context.Context, iso2, iso3, slug, currency string) (CountryRow, error)
	UpdateCountry(ctx context.Context, iso2, iso3, slug, currency string) (CountryRow, error)
	...
	// Events — catalog occurrences; dates and venues live on sessions (AB-36/AB-37)
	InsertEvent(ctx context.Context, orgID uuid.UUID, name string, description *string, status, visibility string, imageURL *string) (EventRow, error)
	...
	// GetTicketPresentationByID resolves the order / event / session / venue /
	// tier / holder / price / poster values the ticket e-mail and PDF print,
	// at render time.
