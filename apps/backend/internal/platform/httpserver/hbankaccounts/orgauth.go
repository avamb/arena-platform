// orgauth.go provides org-scoped membership enforcement for the hbankaccounts
...
// member of orgID. On failure it writes the appropriate HTTP error and returns
// false; the caller must stop handler execution when false is returned.
...
func (h *Handler) requireOrgMembership(w http.ResponseWriter, r *http.Request, orgID uuid.UUID) bool {
	...
	if err != nil {
		h.logger.Error("bank_account: org membership check failed", "error", err.Error())
		httputil.WriteJSON(w, http.StatusInternalServerError, httputil.ErrorEnvelope(
			"bank_account.membership_check_failed", "failed to verify org membership", r,
		))
		return false
	}
	if !member {
