// orgauth.go provides org-scoped membership enforcement for the hapikeys
// handlers, mirroring hbankaccounts/orgauth.go.
package hapikeys
...
func actorIsMemberOfOrg(ctx context.Context, q *gen.Queries, orgID uuid.UUID) (bool, error) {
	...
	// reach is exactly api_keys.org_id. api_key.manage is a forbidden exact
	// scope (apikeys.ForbiddenScopeExact) so a service actor can never
	// legitimately pass this check, but the generic decision still applies
	// here for consistency with every other org-scoped handler package.
...
// requireOrgMembership verifies that the authenticated actor is an active
// member of orgID. On failure it writes the appropriate HTTP error and
// returns false; the caller must stop handler execution when false is
// returned. When membershipQueries is nil (test environments without the
// field wired), the check is skipped and true is returned.
func (h *Handler) requireOrgMembership(w http.ResponseWriter, r *http.Request, orgID uuid.UUID) bool {
	...
	if err != nil {
		h.logger.Error("api_key: org membership check failed", "error", err.Error())
		httputil.WriteJSON(w, http.StatusInternalServerError, httputil.ErrorEnvelope(
			"api_key.membership_check_failed", "failed to verify org membership", r,
		))
		return false
	}
	if !member {
