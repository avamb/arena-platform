...
func (h *Handler) IssueTicketsForCheckoutWithOptions(ctx context.Context, cs gen.CheckoutSessionRow, opts IssuanceOptions) ([]gen.TicketRow, error) {
	...
	// ── EAN-13 credential + barcode (feature #502, W1-B6a; random-EAN13
	// change) ─────────────────────────────────────────────────────────────
	...
	//
	// mint.EAN13 draws a random candidate and atomically claims the
	// barcodes row FIRST, retrying (bounded) on a collision with ANY
	// authority — never a raw 23505 inside this transaction (AGENTS.md
	// "best effort writes inside a money transaction"). Only once that row
	// is won do we write the dependent ticket_credentials row with the
	// same payload.
	if len(newTickets) > 0 {
		...
		for _, t := range newTickets {
			ticketID := t.ID
			code, err := mint.EAN13(ctx, txQ, platformAuthority.ID, &ticketID, nil)
			if err != nil {
				return nil, fmt.Errorf("IssueTicketsForCheckout: mint ean13 barcode for ticket %s: %w",
					t.ID.String(), err)
			}
