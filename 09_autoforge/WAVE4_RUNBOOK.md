...
**Stand 2026-08-23 (evening): backend + admin redeployed on `485a6a8`** (pass 8 + review docs; CI run 32615511617 green; migrations 89; api/worker healthy). Previous: **backend + admin on `c11bb66`** (pass 7 re-committed clean + review repairs; CI run 32602378494 green, all 9 jobs; migrations unchanged at 89; api/worker healthy; healthz/readyz 200). NOTE: `master` was force-pushed (owner decision, `--force-with-lease`) to drop AutoForge's 5e235a7 history carrying 9.7k cache files, a 13.5 MB binary and a local JWT — anyone with an older clone must `git fetch && git reset --hard origin/master`. Previous stand state: **2026-08-22 on `e156b1b`** (pass 5 + repaired pass 6; CI run 32569565952 green, all 9 jobs). Migrate 84→89 clean on the stand DB (0086 cancellation, 0087 tier prices + btree_gist, 0088 MACS ids, 0089 MACS subscribers); api/worker healthy; healthz/readyz 200 via Cloudflare; admin rebuilt from Git master. Procedure unchanged (Raw-compose tag ×4 via `.cm-content`.cmTile.view.dispatch → Save → Deploy → Confirm; admin → Deploy → Confirm).
