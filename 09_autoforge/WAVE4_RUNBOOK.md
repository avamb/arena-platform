...
| 4 | AutoForge | AB-39, AB-40D (+AB-47b/c #435/#436) | **implemented by AutoForge 2026-08-05, REVIEWED + REPAIRED 2026-08-21 in the interactive session.** Verdicts: AB-40D pass; AB-47b/c pass-with-deviations; AB-39 pass after fixes. The batch was left UNPUSHED with real defects the interactive review caught: (a) AB-47b's migration 0085 + session_media queries/gen were never `git add`ed — committed tree didn't build (fixed 02bb077); (b) `BulkSetSessionSeatTier` had no status/kind gate — a seat turning held/sold between pre-check and UPDATE was silently re-priced, and AB-51 GA units were re-tierable, corrupting pool accounting (fixed: column-side gate + rows-affected 409 + 422 ga_unit_not_reassignable); (c) public schema ETag was geometry-only so reassignment never invalidated cached category_prices (fixed: ETag = checksum + seat_status_version); (d) AB-47b let org A attach org B's media by UUID (fixed: org check, cross-org reads as not-found); (e) 13 undocumented OpenAPI items, `nullable:` OAS-3.0 keyword, 2 guardrail-178 context.Background rollbacks, gofmt ×4, govet unusedwrite ×1 — all pre-push-gate failures AutoForge never ran. Recorded follow-ups: widget does not render the AB-47b gallery (data-only); PDF/email cover ambiguity (AB-47c step 2); per-seat reassignment of a category SUBSET still invisible to the widget (first-seat-wins category anchoring) — fold into AB-48's price-surface rework |
| 5 | interactive (Fable 5) | AB-49, AB-48, AB-41 | **IN PROGRESS 2026-08-22.** AB-49 DONE (f60e4dd, migration 0086): operator cancel `POST /v1/tickets/{id}/cancel` (seat/GA-unit release + capacity + barcode revoke + audit + `v1.ticket.cancelled` outbox event in one tx; refund none/manual/automatic strictly after commit), webhook demoted to defensive trigger (full refund releases seats; partial → review_hold, cancels nothing), comp revocation releases seats, admin console cancel UI. AB-48 DONE (migration 0087): `ticket_tier_prices` + btree_gist EXCLUDE, ONE resolver `internal/domain/pricing` fronted by `httpserver/priceresolve` and wired into quote / checkout confirm / all reservation creations / public feed / widget schema / Bil24; **price lock = `reservation_ga_items` lines for EVERY reservation shape** (seated + single-tier GA now write them; checkout charges the stored unit_price); GET/PUT `.../tiers/{id}/price-schedule` + `POST .../sessions/pricing-bulk` (name-matched grid, per-session tx, audited); price edits audited (`v1.tier.price.update`); tier list carries seat_count/ga_unit_count; feed/schema expose `current_price` + `next_price_change_at`; widget charges current_price and shows "price changes on <date>". **Decisions:** gap policy = fall back to base price (documented in 0087 + domain/pricing); AB-40 C2 "several sessions in one pass" delivered as the SessionEditor "Additional dates" repeater (one atomic POST per date, partial progress reported) — no bulk create endpoint, the session-create handler is monolithic and a server-side loop would buy nothing the client loop doesn't. AB-41 DONE: `hcheckout/provider_config.go` — `sales_channels` still decides WHICH provider, `payment_provider_configs` now supplies the go/no-go + credentials: `POST /v1/payment-intents` derives the provider from the checkout's channel (client value may be omitted; a contradicting one → 422 `payment.provider_mismatch`) and `POST /v1/checkout/{id}/complete` validates `payment_provider` against the channel; both require an active+configured config for the org (422 `payment.provider_not_configured|provider_inactive|provider_missing_required_fields|provider_kyb_required`, never a default); inbound webhook HMAC verification prefers the org's config secret (`stripe.webhook_secret` / `allpay.secret_key`, located via refund_id / payment_intent_id) over the process-env secret, which stays the platform fallback. **KYB decision (step 5):** `organizations.kyb_status` gates going LIVE — a `mode=live` config is only usable (checkout) and only activatable (POST/PATCH payment-configs → 422 `payment_config.kyb_required_for_live`) when the org is `verified`; test-mode configs are unrestricted. Note: no code path instantiates the Stripe adapter today (the platform records intents and consumes webhooks), so "Stripe reads keys from the config" is realised at the only live Stripe touchpoint — webhook verification — and the config resolver is the single place an adapter-per-org factory must read from when one is wired. **Pass 5 CLOSED 2026-08-22: CI run 32535405947 green (all 9 jobs), head 559825a, migration head 0087.** Stand NOT yet redeployed (migrations 85→87 pending on the stand) |
| 6 | AutoForge | AB-50 (split #437–#440), AB-45 | **implemented by AutoForge 2026-08-22 (4 commits 39cd256 9ae22c2 e46cd58 522a8a3 + 4 "integration:" gate commits), REVIEWED + BLOCKERS REPAIRED in the interactive session 2026-08-22.** Again left UNPUSHED. Review verdicts: AB-50a pass (wrong holderStatus enum 1/2/3/4 → fixed to 0/1/2/3); AB-45 pass-with-deviations (**event_artists CRUD never checked the event belongs to the org → fixed**); **AB-50b FAIL** (macs-export readable cross-tenant → fixed; `tc.cred_type` join referenced a non-existent column so the export query had never run → fixed to `tc.type='static_qr'`; NULL order totals crashed the scan → COALESCE; revoked tickets were dropped → included as holderStatus 3); **AB-50c FAIL** (webhook `data` was `{ticketId}` which MACS's Pydantic Ticket model rejects with 422 → data is now the export Ticket via one builder; v1.ticket.refunded / v1.ticket.revoked carried no session_id and were silently dropped → session resolved from the ticket; envelope id reused the ticket id → now the outbox row id; `created` without TZ → RFC3339 UTC; retry window 31 min → 24 h (MaxAttempts 30); 3xx counted as success → 2xx only); **AB-50d FAIL** (only 1 of 3 scan endpoints gated → all three gate on ticket status incl. transferred; stub stores nothing / round-trip asserted envelope arrival only → left as pass-7 #441); admin "MACS Export" was a bare `<a>` without the Bearer token → authed fetch + blob download. Remaining gaps queued as pass-7 AutoForge features #441 AB-50e (faithful stub + true round-trip), #442 AB-50f (sold price, discountReason, venue-local showTime, GA seatId, export tests, audit), #443 AB-45b (promo tier on public checkout, PATCH-safe metadata, logo write path, audit) via `import_pass7_fix_features.py` |
| 7 | AutoForge | AB-50e, AB-50f, AB-45b (#441–#443) | **implemented by AutoForge 2026-08-22, REVIEWED 2026-08-22 — all three FAIL on acceptance; money bugs repaired interactively, rest re-queued as pass 8.** Batch was again unpushed, and the AB-45b commit swept in 9.7k files (`.golangci-cache`, `.tmp`, a 13.5 MB binary, Go telemetry, scratch txt, Playwright XML, a worktree gitlink and a local JWT `.tmp-token.json`) — history rewritten before push (soft-reset + clean re-commit e529e88; the junk never reached origin). Verdicts: **AB-50e FAIL** (round-trip cancels via raw UPDATE and calls Dispatch() by hand — no real handler, no outbox/dead-letter path; stub validation WEAKENED to pass a city-less fixture; fabrication partial; dead panic helpers); **AB-50f FAIL** (Order.sum doubled, GA seatId collision untouched, discount/charge hardcoded, no golden file, 'Внешняя система' missing, seated sold price still the current tier price); **AB-45b FAIL** (confirm path split the subtotal into EQUAL shares per tier → wrong discount base; validate pre-flight multiplied the eligible subtotal by the number of tiers; recovery path not migrated; zero tests; no backlog decision). **Repaired here:** confirm uses real per-line amounts (`TierLinesFromPricingLines`), validate computes once and an empty `tier_ids` never bypasses a restriction, recovery uses the one helper, `Order.sum` fixed, GA seatId in a disjoint 1e9+ range, OpenAPI text corrected. Re-queued as pass 8 #444 AB-50g / #445 AB-50h / #446 AB-45c (`import_pass8_fix_features.py`) |
| 8 | AutoForge | AB-50g, AB-50h, AB-45c (#444–#446) | **implemented + self-pushed by AutoForge 2026-08-23 (cd1ea7a b37d008 00e18c4 73fcaf2, CI run 32607703714 green), REVIEWED 2026-08-23: all three pass-with-deviations, no regressions, no junk.** AB-50g: round-trip now goes through the REAL `POST /v1/tickets/{id}/cancel` and the REAL OutboxEventsDispatcher (stub 503→200, next_attempt_at asserted, no dead-letter, stored holderStatus 3/0/0); strict stub validation restored; export 422 `macs.export_incomplete` on a city-less venue — but importer fabrication is still not stored/asserted and the 422 path is untested. AB-50h: golden file is a synthetic fixture and non-binding (logs, never fails); endpoint itself untested (builder called directly); discountReason = "Промокод <code>" (promo_codes has no name column) / "Внешняя система" / "" for plain sales; proration truncates. AB-45c: helper tests exist, tri-state metadata + logo-clear shipped with zero tests, no handler-level promo tests. Interactive follow-up here: OpenAPI 404/422 on macs-export + `discountReason` documented; gofmt. Remaining items queued as pass 9 #447–#449 |
| 9 | AutoForge | AB-50i, AB-45d, docs (#447–#449) | **implemented + self-pushed by AutoForge 2026-08-24 (9011947 6233bfd d63f99b e5d46aa), CI run 32673657201 green (all 9 jobs), REVIEWED 2026-08-24: all three pass, wave 4 CLOSED.** AB-50i: stub fabrication stored + 5 tests; macs-export handler tests 200/422/404 (incl. tenant 404); golden-file test now BINDING (Errorf on divergence); 50e test deleted as superseded by 50g; proration remainder — last ticket absorbs rounding so sum(ticket.discount)==order.discount. AB-45d: tri-state metadata tests (348 lines), logo tests + superadmin null=clear fixed in hiam/admin_orgs.go, promo validate tests, OpenAPI event-metadata fields + codegen (no drift). Docs: MACS contract reconciled + ops runbook, WP contract AB-48/49, AGENTS.md conventions. Deviations (minor, accepted): promo validate tests SIMULATE handler logic rather than call the handler; no handler-level public-checkout/recovery promo test (shared `ValidatePromoForLines` is covered by pass-8 MixedCart tests; both paths route through it); synthetic-sample disclaimer was missing → added interactively in 5740fa5. **OPEN ASK: owner must provide a real MACS sample_tickets.json to replace the synthetic golden fixture.** |

**Stand 2026-08-24: backend + admin redeployed on `4d2492f`** (pass 9 accepted + review follow-ups; CI run 32674646382 green; migrations unchanged at 89 — migrate exited 0; api/worker healthy, healthz/readyz 200 via Cloudflare, admin rebuilt from Git in 36s). Previous: **2026-08-23 (evening): backend + admin redeployed on `485a6a8`** (pass 8 + review docs; CI run 32615511617 green; migrations 89; api/worker healthy). Previous: **backend + admin on `c11bb66`** (pass 7 re-committed clean + review repairs; CI run 32602378494 green, all 9 jobs; migrations unchanged at 89; api/worker healthy; healthz/readyz 200). NOTE: `master` was force-pushed (owner decision, `--force-with-lease`) to drop AutoForge's 5e235a7 history carrying 9.7k cache files, a 13.5 MB binary and a local JWT — anyone with an older clone must `git fetch && git reset --hard origin/master`. Previous stand state: **2026-08-22 on `e156b1b`** (pass 5 + repaired pass 6; CI run 32569565952 green, all 9 jobs). Migrate 84→89 clean on the stand DB (0086 cancellation, 0087 tier prices + btree_gist, 0088 MACS ids, 0089 MACS subscribers); api/worker healthy; healthz/readyz 200 via Cloudflare; admin rebuilt from Git master. Procedure unchanged (Raw-compose tag ×4 via `.cm-content`.cmTile.view.dispatch → Save → Deploy → Confirm; admin → Deploy → Confirm).
